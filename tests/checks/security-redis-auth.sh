...
if [[ -z "$REDIS_PASSWORD" ]]; then
    ...
    if result=$(docker compose exec -T kv redis-cli ping 2>&1); then
        if [[ "$result" == "PONG" ]]; then
            ...
            echo "BROKEN|redis_auth|No password set but ping failed: $result|docker exec kv redis-cli ping"
        fi
    else
        echo "BROKEN|redis_auth|Cannot connect to Redis container|docker exec kv redis-cli ping"
    fi
else
    ...
    if [[ "$unauth_result" == *"NOAUTH"* || "$unauth_result" == *"Authentication required"* ]]; then
        ...
        echo "BROKEN|redis_auth|Authentication configuration unclear|docker exec kv redis-cli ping"
    fi
fi
